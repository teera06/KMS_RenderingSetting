...
public:
	...
	// 생각이 공상가식 프로그래밍.
	// 하드디스크에 있는 이미지를 로드한다.
	std::shared_ptr<UWindowImage> LoadImg(std::string_view _Path);

	std::shared_ptr<UWindowImage> LoadImg(std::string_view _Path, std::string_view _Name);

	void UnloadImg(std::string_view _Name);

	bool IsImage(std::string_view _Name);

	std::shared_ptr<UWindowImage> FindImg(std::string_view _Name);

	// 그 이미지가 없었는데 새롭게 만드는 기능.
	std::shared_ptr<UWindowImage> LoadFolder(std::string_view _Path);

	// 그 이미지가 없었는데 새롭게 만드는 기능.
	std::shared_ptr<UWindowImage> LoadFolder(std::string_view _Path, std::string_view _Name);
...
private: 
	...
	std::map<std::string, std::shared_ptr<UWindowImage>> Images;
